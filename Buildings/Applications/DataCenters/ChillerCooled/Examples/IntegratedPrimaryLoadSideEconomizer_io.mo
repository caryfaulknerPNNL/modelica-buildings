...
model IntegratedPrimaryLoadSideEconomizer_io
  ...
  extends
    ...
    break connect(TCWSupSet.y, cooTowSpeCon.TCWSupSet),
    break connect(dpSetSca.y, pumSpe.u_s),
    roo(QRoo_flow=700000));
  ...
  Modelica.Blocks.Interfaces.RealInput u_CHW_DP "CHW pump DP setpoint"
    annotation (Placement(transformation(extent={{-400,80},{-360,120}})));
  Modelica.Blocks.Interfaces.IntegerInput u_CooModCon "Cooling Mode Controller"
    annotation (Placement(transformation(extent={{-400,40},{-360,80}})));
  Modelica.Blocks.Interfaces.RealInput u_ChiSta "Chiller staging signal"
    annotation (Placement(transformation(extent={{-400,0},{-360,40}})));
  Modelica.Blocks.Interfaces.RealInput u_QRoo "Room heat load"
    annotation (Placement(transformation(extent={{-400,-40},{-360,0}})));
  Modelica.Blocks.Sources.Constant QRoo(k=-50000000)
    annotation (Placement(transformation(extent={{-262,-140},{-242,-120}})));
  Modelica.Blocks.Sources.IntegerConstant integerConstant(k=2)
    annotation (Placement(transformation(extent={{-316,62},{-296,82}})));
  Modelica.Blocks.Sources.Constant dpSetSca1(k=15.5 + 273.15)
    "Scaled differential pressure setpoint"
    annotation (Placement(transformation(extent={{-288,212},{-268,232}})));
  Modelica.Blocks.Sources.IntegerConstant integerConstant1(k=3)
    annotation (Placement(transformation(extent={{-266,78},{-246,98}})));
equation
  ...
  connect(cooModCon.y, sigCha.u) annotation (Line(points={{-193,110},{-238,110},
          {-238,160},{-126,160},{-126,280},{118,280},{118,160},{178,160}},
        color={255,127,0}));
  connect(cooModCon.y, CWPumCon.cooMod) annotation (Line(points={{-193,110},{
          -188,110},{-188,75},{-174,75}}, color={255,127,0}));
  connect(cooModCon.y, cooTowSpeCon.cooMod) annotation (Line(points={{-193,110},
          {-188,110},{-188,182.444},{-172,182.444}}, color={255,127,0}));
  connect(cooModCon.y, intToBoo.u)
    annotation (Line(points={{-193,110},{-172,110}}, color={255,127,0}));
  connect(cooModCon.y, chiStaCon.cooMod) annotation (Line(points={{-193,110},{
          -188,110},{-188,146},{-172,146}}, color={255,127,0}));
  connect(cooLoaChi.y, chiStaCon.QTot)
    annotation (Line(points={{-299,140},{-172,140}}, color={0,0,127}));
  connect(u_CWST, cooTowSpeCon.TCWSupSet) annotation (Line(points={{-380,140},{
          -344,140},{-344,204},{-180,204},{-180,186},{-172,186}}, color={0,0,
          127}));
  connect(u_CHW_DP, pumSpe.u_s) annotation (Line(points={{-380,100},{-328,100},
          {-328,14},{-248,14},{-248,-20}}, color={0,0,127}));
  connect(u_QRoo, roo.uQflow) annotation (Line(points={{-380,-20},{-348,-20},{
          -348,-52},{-332,-52},{-332,-188},{-48,-188},{-48,-168},{-16,-168},{
          -16,-164},{24,-164},{24,-178},{16,-178}}, color={0,0,127}));
  connect(u_CooModCon, cooModCon.uCooModCon) annotation (Line(points={{-380,60},
          {-324,60},{-324,92},{-276,92},{-276,116},{-216,116}}, color={255,127,
          0}));
...
end IntegratedPrimaryLoadSideEconomizer_io;
