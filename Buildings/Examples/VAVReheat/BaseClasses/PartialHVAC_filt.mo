...
partial model PartialHVAC_filt
  ...
  Modelica.Blocks.Interfaces.BooleanOutput y_on_off
    "signal if the system is on or off" annotation (Placement(transformation(
          extent={{440,-124},{460,-104}}), iconTransformation(extent={{440,-124},
            {460,-104}})));
  ...
  Fluid.Sensors.RelativeHumidityTwoPort senRelHumSup(m_flow_nominal=
...
equation
  ...
  connect(damRet.port_b, TMix.port_a)
    annotation (Line(points={{-5.55112e-16,-20},{-5.55112e-16,-40},{10,-40}},
                                                        color={0,127,255}));
  ...
  connect(filt.port_b, inDucGUV.port_a)
    annotation (Line(points={{60,-40},{68,-40}}, color={0,127,255}));
  connect(inDucGUV.port_b, heaCoi.port_a2)
    annotation (Line(points={{88,-40},{98,-40}}, color={0,127,255}));
  connect(y_on_off, inDucGUV.u) annotation (Line(points={{450,-114},{60,-114},{
          60,-48},{66,-48}},           color={255,0,255}));
  connect(senRetFlo.port_b, senRelHumRet.port_a)
    annotation (Line(points={{340,140},{240,140}}, color={0,127,255}));
  connect(senRelHumRet.port_b, TRet.port_a)
    annotation (Line(points={{220,140},{110,140}}, color={0,127,255}));
  connect(TSup.port_b, senRelHumSup.port_a) annotation (Line(points={{350,-40},{
          364,-40},{364,-40}}, color={0,127,255}));
  connect(senRelHumSup.port_b, senSupFlo.port_a)
    annotation (Line(points={{384,-40},{400,-40}}, color={0,127,255}));
  annotation (
  Diagram(
    coordinateSystem(
